...
def mutate_func(parent_network_def,
                m_prob,
                device,
                only_meso,
                only_macro,
    ...
    while count < max_loop_num:
        ...
        skip_weightsss, norm_op_act_weightssss, depths, widths, ratios = arch_generate(
            device,
            generate_meso=not only_macro,
            generate_macro=not only_meso,
...
def mutate_network_def(parent_network_def, m_prob, device, only_meso, only_macro, pre_defined_arch, target, target_flops, target_params, thr):
    '''
        Call mutate_func() to mutate network_def and check whether 
        the generated network_def has too low resource.
    '''
    m_network_def = mutate_func(parent_network_def, m_prob, device, only_meso, only_macro, pre_defined_arch, target, target_flops, target_params, thr)

    return m_network_def


def crossover_func(m_network_def,
                   f_network_def,
                   m_prob,
                   only_meso,
                   only_macro,
...
def crossover_network_def(m_network_def, f_network_def, m_prob, device, only_meso, only_macro, pre_defined_arch, target, target_flops, target_params, thr):
    '''
        Call crossover_func() to generate network_def and check whether 
        the generated network_def has too low resource.
    '''
    c_network_def = crossover_func(m_network_def, f_network_def, m_prob, only_meso, only_macro, pre_defined_arch, target, target_flops, target_params, thr)
