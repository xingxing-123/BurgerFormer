...
parser.add_argument('--layer_scale_init_value', type=float, default=1e-5)
parser.add_argument('--net_config', default='poolformer_s12', type=str)
...
def main():
    ...
    dataset_train = create_dataset(args.dataset, root=args.data_dir, split=args.train_split, is_training=True, class_map=args.class_map, download=args.dataset_download, batch_size=args.batch_size, repeats=args.epoch_repeats)
...
def train_one_epoch(epoch, model, loader, optimizer, loss_fn, args, lr_scheduler=None, saver=None, output_dir=None, amp_autocast=suppress, loss_scaler=None, model_ema=None, mixup_fn=None):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if args.accumulate_grad == 1:
            ...
            loss.backward(create_graph=second_order)
            if batch_idx % args.accumulate_grad == 0 or last_batch:
                if args.clip_grad is not None:
                    dispatch_clip_grad(model_parameters(model, exclude_head='agc' in args.clip_mode), value=args.clip_grad, mode=args.clip_mode)
                optimizer.step()
                optimizer.zero_grad()
