...
from evolver import PopulationEvolver
...
def get_args_parser():
    ...
    parser.add_argument('--net_config', default='myburger', type=str, help='arch_name')
    ...
    # Dataset parameters
    parser.add_argument('--data-dir', default='/YOUR_PATH', type=str, help='dataset path')
    ...
    # for searching
    parser.add_argument('--no-use-holdout',
                        action='store_false',
                        dest='use_holdout',
                        default=True,
    ...
    parser.add_argument('--init-popu-size',
                        default=500,
                        type=int,
            ...
            if last_batch:
                ...
                if args.local_rank == 0:
                    _log.info('{}: '
                              ...
                              'flops/G: {:>7.4f}  '
                              'params/M: {:>7.4f}  \n'.format(log_name,
                                                              time.time() - start_time, top1_m.avg, top5_m.avg, flops / 1e9, params / 1e6))
...
def main(args):
    ...
    for search_iter in range(args.search_iter):
        ...
        for subnet_idx in range(len(popu_evolve.popu)):
            ...
            test_stats = validate(model, data_loader_val, sub_network_def[0], sub_network_def[1], sub_network_def[2], sub_network_def[3], sub_network_def[4],
                                  args)
    ...
    for i in range(len(_best_result_history)):
        ...
        if args.local_rank == 0:
            _log.info('Iter [{}]: Acc = {:.2f}, flops = {:.2f}G, params = {:.2f}M Network_def = {}'.format(i, _best_result_history[i].score, flops / 1e9,
                                                                                                           params / 1e6, _best_result_history[i].network_str))

    with open('arch.py', 'a') as f:
