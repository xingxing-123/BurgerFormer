...
class PopulationEvolver():
    ...
    def random_sample(self, num_samples, only_meso, only_macro, pre_defined_arch='', target='flops', target_flops=1.8e9, target_params=12e6, thr=0.975):
        popu_idx = 0
    ...
    def evolve_sample(self, parent_size, mutate_prob, mutate_size, only_meso, only_macro, pre_defined_arch='', crossover_size=None, target='flops', target_flops=1.8e9, target_params=12e6, thr=0.975):
        if self.popu:
        ...
        while popu_idx < mutate_size:
            ...
            network_def = mutate_network_def(parent_network_def, mutate_prob, self.device, only_meso, only_macro, pre_defined_arch, target, target_flops, target_params, thr)
        ...
        while popu_idx < crossover_size:
            ...
            network_def = crossover_network_def(m_network_def, f_network_def, 0.5, self.device, only_meso, only_macro, pre_defined_arch, target, target_flops, target_params, thr)
            new_ind = Individual(network_def[0], network_def[1], network_def[2], network_def[3], network_def[4])
            if (new_ind not in self.popu and new_ind not in self.history_popu) or skip_checking_counter >= skip_checking_threshold:
                self.popu.append(new_ind)
                popu_idx = popu_idx + 1
