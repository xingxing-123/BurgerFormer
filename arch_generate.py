...
def arch_generate(device, generate_meso=True, generate_macro=True, stage=4, multiplier=3, has_atten_mlp=CONFIG['normopact_module']['has_atten_mlp'], search_residual=False, pre_defind_arch=None, min_macro=False, max_macro=False):
    with torch.no_grad():
        ...
        if not generate_meso or not generate_macro:
            ...
            if generate_meso:
                ...
                for j in range(multiplier):
                    if j == 1:
                        ...
                        # op_op_num = bread_op_num - 1 # no spatial mlp
    ...
    with torch.no_grad():
        ...
        for i in range(stage):
            ...
            for j in range(multiplier):
                ...
                stage_dict['meso'][str(j + 1)] = '{}-{}-{}-{}-{}'.format(
                ...
                if stage_dict['meso'][str(j + 1)] == 'skip-skip-skip-skip-skip' and j != 1:
                    stage_dict['meso']["use_layer_scale"][j] = False
                    stage_dict['meso']["is_drop"][j] = False
            pos = 0
            for j in range(1, multiplier + 1):
                for k in range(j):
                    idx = skip_weightss[pos].argmax().item()
                    pos += 1
                    stage_dict['meso']['{}->{}'.format(k, j)] = CONFIG['skip_module'][idx]
...
def arch2_weight(arch, device, stage=4, multiplier=3, has_atten_mlp=CONFIG['normopact_module']['has_atten_mlp']):
    ...
    for i in range(stage):
        ...
        meso_arch = arch['stage-{}'.format(i + 1)]['meso']
        pos = 0
        for j in range(multiplier):
            if j == 1:
                op_op_num = meat_op_num
            ...
            ops = meso_arch['{}'.format(j + 1)].split('-')
            ...
            for k in range(j + 1):
                key = '{}->{}'.format(k, j + 1)
                idx = CONFIG['skip_module'].index(meso_arch[key])
...
if __name__ == "__main__":
    ...
    poolformer_s12 = {
        'stage-1': {'macro': {'depth': 2, 'width': 64, 'ratio': 4}, 'meso': {'use_layer_scale': [True, True, False], 'is_drop': [True, True, False], "1": "skip-GN-avgpool-skip-skip", "2": "skip-skip-skip-skip-skip", "3": "skip-skip-skip-skip-skip", "0->1": "skip", "0->2": "none", "1->2": "skip", "0->3": "none", "1->3": "none", "2->3": "none"}}, 
        'stage-2': {'macro': {'depth': 2, 'width': 128, 'ratio': 4}, 'meso': {'use_layer_scale': [True, True, False], 'is_drop': [True, True, False], "1": "skip-GN-avgpool-skip-skip", "2": "skip-skip-skip-skip-skip", "3": "skip-skip-skip-skip-skip", "0->1": "skip", "0->2": "none", "1->2": "skip", "0->3": "none", "1->3": "none", "2->3": "none"}}, 
        'stage-3': {'macro': {'depth': 6, 'width': 320, 'ratio': 4}, 'meso': {'use_layer_scale': [True, True, False], 'is_drop': [True, True, False], "1": "skip-GN-avgpool-skip-skip", "2": "skip-skip-skip-skip-skip", "3": "skip-skip-skip-skip-skip", "0->1": "skip", "0->2": "none", "1->2": "skip", "0->3": "none", "1->3": "none", "2->3": "none"}}, 
        'stage-4': {'macro': {'depth': 2, 'width': 512, 'ratio': 4}, 'meso': {'use_layer_scale': [True, True, False], 'is_drop': [True, True, False], "1": "skip-GN-avgpool-skip-skip", "2": "skip-skip-skip-skip-skip", "3": "skip-skip-skip-skip-skip", "0->1": "skip", "0->2": "none", "1->2": "skip", "0->3": "none", "1->3": "none", "2->3": "none"}}
    }

    skip_weightsss, norm_op_act_weightssss, depths, widths, ratios = arch_generate(device, generate_meso=True, generate_macro=False, pre_defind_arch=poolformer_s12)
