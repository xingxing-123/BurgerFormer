...
import supernet
from arch_generate import arch_generate, arch2_weight, weight2_arch, del_vars, arch_generate_multipath
import arch as arch
...
parser.add_argument('--only_meso', action='store_true', default=False)
...
parser.add_argument('--multipath_num', type=int, default=1)
parser.add_argument('--sandwich', action='store_true', default=False)
...
parser.add_argument('--aug-repeats', type=int, default=0, help='Number of augmentation repetitions (distributed training only) (default: 0)')
...
parser.add_argument('--checkpoint-hist', type=int, default=2, metavar='N', help='number of checkpoints to keep (default: 10)')
...
def main():
    ...
    # assert args.multipath_num == 1 or (args.multipath_num > 1 and args.sandwich == False)
...
def train_one_epoch(epoch,
    ...
    for batch_idx, (input, target) in enumerate(loader):
        try:
            ...
            assert loss_scaler is None
            for _ in range(args.grad_accumulate_step):
                with amp_autocast():
                    if not args.distributed:
                        skip_weightsss, norm_op_act_weightssss, depths, widths, ratios = arch_generate_multipath(
                            input.device,
                            generate_meso=not args.only_macro,
                            generate_macro=not args.only_meso,
                            pre_defind_arch=None if args.pre_defined_arch == '' else eval('arch.{}'.format(args.pre_defined_arch)),
                            path_num=args.multipath_num,
                        )
                    else:
                        if dist.get_rank() == 0:
                            skip_weightsss, norm_op_act_weightssss, depths, widths, ratios = arch_generate_multipath(
                            ...
                            objects = [skip_weightsss, norm_op_act_weightssss, depths, widths, ratios]
                        else:
                            objects = [None, None, None, None, None]
                        dist.broadcast_object_list(objects, src=0)
                        skip_weightsss, norm_op_act_weightssss, depths, widths, ratios = objects
                    output = model(input, skip_weightsss, norm_op_act_weightssss, depths, widths, ratios)
                    loss = loss_fn(output, target)
                if not args.distributed:
                    losses_m.update(loss.item(), input.size(0))
                loss.backward(create_graph=second_order)
                # print(args.rank, batch_idx)

                if args.hybrid:
                    sampled_arch = weight2_arch(skip_weightsss, norm_op_act_weightssss, depths, widths, ratios)
                    # min macro
                    with amp_autocast():
                        _, _, depths, widths, ratios = arch_generate(input.device,
                                                                     generate_meso=False,
                                                                     generate_macro=True,
                                                                     pre_defind_arch=sampled_arch,
                                                                     min_macro=True)
                        output = model(input, skip_weightsss, norm_op_act_weightssss, depths, widths, ratios)
                        loss = loss_fn(output, target)
                    if not args.distributed:
                ...
                if args.sandwich:
                    # min macro
                    with amp_autocast():
                        skip_weightsss, norm_op_act_weightssss, depths, widths, ratios = arch2_weight(eval('arch.min_arch'), input.device)
                        output = model(input, skip_weightsss, norm_op_act_weightssss, depths, widths, ratios)
                        loss = loss_fn(output, target)
                    if not args.distributed:
                        losses_m.update(loss.item(), input.size(0))
                    ...
                    # max macro
                    with amp_autocast():
                        skip_weightsss, norm_op_act_weightssss, depths, widths, ratios = arch2_weight(eval('arch.max_arch'), input.device)
                        output = model(input, skip_weightsss, norm_op_act_weightssss, depths, widths, ratios)
                        loss = loss_fn(output, target)
                    if not args.distributed:
                        losses_m.update(loss.item(), input.size(0))
        ...
        # end for
            ...
            sampled_arch = weight2_arch(skip_weightsss, norm_op_act_weightssss, depths, widths, ratios)
            print('rank {}, CUDA OOM: '.format(args.rank), sampled_arch)
            # print(e)
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            try:
                ...
                with amp_autocast():
                    skip_weightsss, norm_op_act_weightssss, depths, widths, ratios = arch_generate(
                        input.device,
                        generate_meso=not args.only_macro,
                        generate_macro=not args.only_meso,
                        pre_defind_arch=None if args.pre_defined_arch == '' else eval('arch.{}'.format(args.pre_defined_arch)))
